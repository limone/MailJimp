 ...
 * 
 * 
 * @author: Eike Hirsch (me at eike-hirsch dot net) Date: 03.05.11 Time: 09:32
 */
public interface IWebHookAdapter extends Serializable {
  /**
   * Checks if this request is secure. It's a good idea to manually add a secret
   * parameter to the WebHooks url and test that parameter and it's value here.
   * You can always add a parameter at the list tools section in you MailChimp
   * account. Simply add something like <code>?secretKey=secretValue</code> to
   * the url.
   * 
   * @param request
